      ...
      <div className="w-full max-w-3xl">
        <div className="flex justify-between items-center mb-8 gap-4">
          ...
          <div className="flex gap-2">
            <Link href="/presets" className="bg-gray-800 px-4 py-2 rounded hover:bg-gray-700 font-semibold">Presets</Link>
          ...
          {characters.map((char) => (
            <Link key={char.id} href={`/character/${char.id}`} className="bg-gray-900 rounded-lg p-4 shadow hover:shadow-lg transition border border-gray-800 flex flex-col">
              <div className="flex gap-2 mb-2">
                {(char.imagePaths || []).slice(0, 2).map((img, i) => <img key={i} src={img} alt="ref" className="w-16 h-16 object-cover rounded" />)}
              </div>
